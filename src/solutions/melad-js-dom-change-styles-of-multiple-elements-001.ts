const MeladJsDomChangeStylesOfMultipleElements001 = () => {
	...
	setTimeout(() => {
		...
		if (authors18Elems) {
			for (const authors18Elem of authors18Elems) {
				authors18Elem.style.color = 'blue';
			}
		}

		if (authors19Elems) {
			for (const authors19Elem of authors19Elems) {
				authors19Elem.style.color = 'white';
			}
		}
	});
...
export default MeladJsDomChangeStylesOfMultipleElements001;